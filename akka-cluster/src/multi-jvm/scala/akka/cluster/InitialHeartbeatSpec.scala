...
abstract class InitialHeartbeatSpec
  ...
  "A member" must {
    ...
    "must detect failure even though no heartbeats have been received" taggedAs LongRunningTest in {
        ...
        cluster.subscribe(system.actorOf(Props(new Actor {
          def receive = {
            case state: InstantClusterState ⇒
              if (state.members.map(_.address) contains cluster.selfAddress)
                joinLatch.countDown()
      ...
      runOn(controller) {
        ...
        testConductor.shutdown(first, 0).await
